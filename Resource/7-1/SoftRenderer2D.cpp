...
float fovAngle = 60.f; // 플레이어 시야각 설정
...
void SoftRenderer::Update2D(float InDeltaSeconds)
{
	...
	Vector2 f = rMatrix * Vector2::UnitY;
	Vector2 v = (targetPosition - playerPosition).GetNormalize();

	// 물체의 최종 상태 결정
	if (v.Dot(f) >= halfFovCos) // 시야 범위 내
	{
...
void SoftRenderer::Render2D()
{
	...
	r.DrawLine(playerPosition, playerPosition + rMatrix * Vector2(sightLength * halfFovSin, sightLength * halfFovCos), playerColor);
	r.DrawLine(playerPosition, playerPosition + rMatrix * Vector2(-sightLength * halfFovSin, sightLength * halfFovCos), playerColor);
	r.DrawLine(playerPosition, playerPosition + rMatrix * Vector2::UnitY * sightLength * 0.2f, playerColor);
	...
	r.PushStatisticText(std::string("Player Degree : ") + std::to_string(currentDegree));
